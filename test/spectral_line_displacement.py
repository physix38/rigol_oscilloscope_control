...
运行实例：uv run test\spectral_line_displacement.py --interval 5 --max_displacement 5 --iteration 5 --prominence_groups 0.15-0.3,0.05-0.15 --min_distance_gap 10 --plot
"""
import sys
import os
import time
...
def parse_args():
    ...
    parser.add_argument('--prominence_threshold', type=float, default=0.05,
    ...
    parser.add_argument('--prominence_groups', type=str, default='0.05-0.15,0.15-0.3,0.5-1.0',
                        help='峰突出度分组范围，格式为"min1-max1,min2-max2,..." (默认: "0.05-0.15,0.15-0.3,0.5-1.0")')
    parser.add_argument('--min_distance_gap', type=float, default=10.0,
                        help='组内相邻峰最小距离差距阈值，大于此值才计入总平均位移 (默认: 10.0)')
...
def compute_spectral_line_displacement(displacements, prominence_groups_str, min_distance_gap):
    """
    根据突出度范围分组匹配峰，计算谱线总位移。
    参数:
        displacements: 列表，每个元素是字典，包含 'displacement', 'prominence', 'curr_position', 'curr_idx' 等
        prominence_groups_str: 字符串，例如 "0.1-0.3,0.3-0.5,0.5-1.0"
        min_distance_gap: 浮点数，相邻峰最小距离阈值
    返回:
        weighted_avg_displacement: 加权平均总位移（浮点数）
        group_details: 字典列表，每个元素包含组的信息
    """
    import numpy as np

    # 1. 解析突出度分组
    groups = []
    for part in prominence_groups_str.split(','):
        part = part.strip()
        if not part:
            continue
        if '-' in part:
            low, high = part.split('-')
            groups.append((float(low), float(high)))
        else:
            # 如果只有一个值，视为下限，上限为正无穷
            groups.append((float(part), float('inf')))

    # 2. 初始化分组容器
    group_displacements = [[] for _ in range(len(groups))]
    group_positions = [[] for _ in range(len(groups))]

    # 3. 分配位移到对应组
    for d in displacements:
        prominence = d['prominence']
        for idx, (low, high) in enumerate(groups):
            if low <= prominence < high:
                group_displacements[idx].append(d['displacement'])
                group_positions[idx].append(d['curr_position'])
                break
        # 如果不属于任何组，忽略
    ...
    for idx, (disps, positions) in enumerate(zip(group_displacements, group_positions)):
        if len(disps) == 0:
        ...
        # 计算平均位移
        disps_size=np.size(disps)
        disps1=np.arange(disps_size)
        for i in range (0,disps_size):
            if (i%2==0):
                disps1[i]=1
            else:
                disps1[i]=-1
        disps2=disps*disps1
        avg_disp = np.mean(disps2)
        group_avg.append(avg_disp)
        group_counts.append(len(disps))
        ...
        if len(positions) < 2:
            ...
            if all(gap > min_distance_gap for gap in gaps):
                group_valid.append(True)
            else:
                group_valid.append(False)
    ...
    for idx in range(len(groups)):
        if group_valid[idx] and group_counts[idx] > 0:
            total_weighted_sum += np.absolute(group_avg[idx]) * group_counts[idx]
            total_weight += group_counts[idx]
        group_details.append({
            'group_range': groups[idx],
            'count': group_counts[idx],
            'average_displacement': group_avg[idx],
            'valid': group_valid[idx]
        })

    if total_weight == 0:
        weighted_avg_displacement = 0.0
    else:
        weighted_avg_displacement = total_weighted_sum / total_weight

    return weighted_avg_displacement, group_details


def check_min_two_peaks_same_group(group_positions):
    """
    判断所有在分组内的峰按照 x*10000 轴位置排序时最小的两个峰是否为同一组。

    参数:
        group_positions: 列表的列表，每个子列表包含该组峰的位置 (x*10000 值)

    返回:
        -1: 最小的两个峰属于同一组
         0: 最小的两个峰属于不同组，或峰数量不足两个
    """
    import numpy as np
    # 扁平化并记录组索引
    all_peaks = []
    for group_idx, positions in enumerate(group_positions):
        for pos in positions:
            all_peaks.append((pos, group_idx))
    if len(all_peaks) < 2:
        return 0
    # 按位置排序
    all_peaks_sorted = sorted(all_peaks, key=lambda x: x[0])
    # 取最小的两个
    _, group1 = all_peaks_sorted[0]
    _, group2 = all_peaks_sorted[1]
    if group1 == group2:
        return -1
    else:
        return 0
#problematic
def check_leftmost_peak_change(prev_group_positions, curr_group_positions, tolerance=1e-6):
    """
    判断分组内当前测量时刻与上一次测量时刻相比，是否有峰在最左侧（x*10000轴)消失/产生。

    参数:
        prev_group_positions: 列表的列表，前一次测量时刻各组峰的位置
        curr_group_positions: 列表的列表，当前测量时刻各组峰的位置
        tolerance: 位置变化的容差（默认 1e-6）
    ...
    import numpy as np
    # 提取所有位置
    def extract_all_positions(group_positions):
        positions = []
        for pos_list in group_positions:
            positions.extend(pos_list)
        return positions
    prev_positions = extract_all_positions(prev_group_positions)
    curr_positions = extract_all_positions(curr_group_positions)
    # 寻找最左侧位置
    prev_leftmost = min(prev_positions) if prev_positions else None
    curr_leftmost = min(curr_positions) if curr_positions else None
    # 判断变化
    if prev_leftmost is None and curr_leftmost is None:
        # 两次都无峰，视为无变化
        return 1
    elif prev_leftmost is None or curr_leftmost is None:
        # 一方有峰另一方无，视为有变化
        return -1
    elif abs(prev_leftmost - curr_leftmost) > tolerance:
        # 最左侧位置发生变化
        return -1
    else:
        return 1
...
def find_peaks_above_threshold(csv_file, prominence_threshold, distance=100):
    """
...
def main():
    ...
    try:
        for i in range(1, args.iterations + 1):
            ...
            # 计算分组加权平均位移
            if displacements:
                weighted_avg_displacement, group_details = compute_spectral_line_displacement(
                    displacements, args.prominence_groups, args.min_distance_gap
                )
                avg_displacement = weighted_avg_displacement
            else:
