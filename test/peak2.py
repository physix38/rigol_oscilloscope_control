...
def calculate_peak_differences_enhanced(df, prominence_col=8, position_col=2,
                                        small_prominence_range=(None, None),
                                        large_prominence_range=(None, None)):
    """
    ...
    position_col: 位置列的索引
    small_prominence_range: 元组 (min, max)，小峰突出度的最小值和最大值范围。
        默认 (None, None) 表示无限制。
    large_prominence_range: 元组 (min, max)，大峰突出度的最小值和最大值范围。
        默认 (None, None) 表示无限制。
    ...
    dict: 包含以下键的字典
        ...
        'summary': 统计摘要
        'average_diff': 所有差值的平均值（DataFrame）
        'std': 所有差值的标准差（DataFrame）
    """
    ...
    # 解析范围
    small_min, small_max = small_prominence_range
    large_min, large_max = large_prominence_range
    
    # 将None转换为无穷大/无穷小以方便比较
    ...
    # 寻找满足"小，大，小"模式的峰值，并且符合指定的突出度范围
    for i in range(1, len(prominence) - 1):
        if (small_min <= prominence.iloc[i-1] <= small_max and
            small_min <= prominence.iloc[i+1] <= small_max and
            large_min <= prominence.iloc[i] <= large_max and
            prominence.iloc[i-1] < prominence.iloc[i] and
...
if __name__ == "__main__":
    ...
    print("原始结果（无范围筛选）:")
    print("平均差值:", enhanced_results['average_diff'])
    print("标准差:", enhanced_results['std'])
    ...
    filtered_results = calculate_peak_differences_enhanced(
        ...
        prominence_col=7, 
        position_col=1,
        small_prominence_range=small_range,
        large_prominence_range=large_range
    )
    print("\n使用范围筛选的结果:")
    print("找到的峰数量:", filtered_results['summary']['total_peaks'])
